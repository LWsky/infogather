        ...
        for i in jvm_res:
            ...
            for j in i:
                if "Dcatalina.home" in j:
                    ...
                    jvm_gc_info = dict(jvm_info,**jvm_gc_info)
                    jvm_gc_info["host_name"] = self.hostname
                    jvm_gc_info["ip"] = self.ip
                    jvm_gc_info["create_time"] = self.get_time()
                    jvm_infos[i[1]] = jvm_gc_info
                else:
                    return
        data = jvm_infos
        return data
